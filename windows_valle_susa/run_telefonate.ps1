...
$RouteProspect = Join-Path $PSScriptRoot 'prospect_vie_giro.py'
...
Write-Host '[2/9] Importo nel motore directory gli eventuali contatti locali gia esistenti'
py $Sync export
if ($LASTEXITCODE -ne 0) { throw "Sync database terminato con codice $LASTEXITCODE" }

Write-Host '[3/9] Scarico il bridge locale corrente'
Invoke-WebRequest -UseBasicParsing $BridgeUrl -OutFile $Bridge
if (-not (Test-Path $Bridge)) { throw 'Bridge V3 non scaricato.' }

Write-Host '[4/9] Radar -> microzone -> directory pubbliche -> numeri locali'
py $Bridge
...
Write-Host '[9/9] Installa/aggiorna server persistente, health-check e test restart'
...
Write-Host "PROSPECT WEB LOCALI + VIE GIRO: $WebProspects"
Write-Host 'NOTA: un recapito pubblico non dimostra proprieta immobiliare. Verifica fonte, pertinenza e RPO prima di chiamare.' -ForegroundColor Yellow
