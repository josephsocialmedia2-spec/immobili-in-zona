...
TRACKED_ENGINES={"DDG_MATRIX_V1","PUBLIC_DORK_MATRIX_V3"}

def now():return datetime.now(timezone.utc).isoformat()
def key(s):return hashlib.sha256(s.encode("utf-8")).hexdigest()[:20]
def set_lifecycle(x,value):
    if x.get("lifecycle")!=value:x["lifecycle"]=value;x["last_status_change_at"]=now()
...
for item_id,x in list(items.items()):
    if x.get("discovery_engine") in {"DUCKDUCKGO_HTML","DUCKDUCKGO_HTML_V2","DUCKDUCKGO_AGGREGATED_V3"}:del items[item_id]
...
for r in current:
    ...
    seen_urls.add(url);i=key(url);seen_ids.add(i);p=r.get("price");engine=r.get("discovery_engine") or "PUBLIC_DORK_MATRIX_V3"
    if i not in items:
        items[i]={"id":i,"comune":r.get("comune",""),"fonte":r.get("fonte",""),"url":url,"title":r.get("title","")[:220],"snippet":r.get("snippet","")[:700],"price_history":[],"seller_hint":r.get("seller_hint","NON_DETERMINATO"),"private_intent":bool(r.get("private_intent")),"market_signal":r.get("market_signal","VENDITA"),"query_label":r.get("query_label",""),"first_seen":now(),"last_seen":now(),"checks":1,"lifecycle":"NEW","missed_checks":0,"domain_rule":r.get("domain_rule",""),"path_rule":r.get("path_rule",""),"discovery_engine":engine}
    else:
        x=items[i];previous=x.get("lifecycle","");x["last_seen"]=now();x["checks"]=int(x.get("checks",0))+1;x["missed_checks"]=0
        x["title"]=r.get("title","")[:220] or x.get("title","");x["snippet"]=r.get("snippet","")[:700] or x.get("snippet","");x["fonte"]=r.get("fonte","") or x.get("fonte","");x["private_intent"]=bool(x.get("private_intent")) or bool(r.get("private_intent"));x["discovery_engine"]=engine;x["market_signal"]=merge_signal(x.get("market_signal"),r.get("market_signal"));x["query_label"]=r.get("query_label","") or x.get("query_label","")
        if r.get("seller_hint") and r.get("seller_hint")!="NON_DETERMINATO":x["seller_hint"]=r["seller_hint"]
        if previous in OUT_STATES:x["reentries"]=int(x.get("reentries",0))+1;x["last_reentry_at"]=now();set_lifecycle(x,"RELISTED")
...
for i,x in items.items():
    if i in seen_ids or x.get("discovery_engine") not in TRACKED_ENGINES:continue
    comune=x.get("comune","")
...
with STATUS.open("w",encoding="utf-8-sig",newline="") as f:w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(statuses)
