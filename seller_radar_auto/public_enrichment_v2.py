#!/usr/bin/env python3
"""F1 Seller Radar — public enrichment v2 con DuckDuckGo HTML."""
...
ADDRESS_RE=re.compile(r"\b(?:via|viale|corso|piazza|strada|borgata|frazione|vicolo|largo)\s+[A-Za-zÀ-ÿ0-9'’.\-\s]{2,80}?\s+\d{1,4}(?:\s*/\s*[A-Za-z0-9]+|\s*[A-Za-z])?\b",re.I)
...
def addresses(text):
    ...
    for m in ADDRESS_RE.finditer(clean(text)):
        ...
        if v.casefold() not in seen: seen.add(v.casefold()); out.append(v[:140])
    return out[:5]
...
state=load_json(STATE,{"items":{}}); items=state.get("items") or {}
candidates=sorted(items.items(),key=lambda kv:(0 if kv[1].get("lifecycle")=="NEW" else 1,-int(kv[1].get("score") or 0)))
...
for pos,(item_id,x) in enumerate(candidates,1):
    ...
    e={"checked_at":now(),"private_candidate":private_candidate,"listing_fetch":{"ok":False,"status":0,"error":""},"address_hints":addresses(f"{title} {snippet}"),"seller_name":"","cross_matches":[],"public_contacts":[],"review_candidates":[],"queries":[]}
    if url:
        ok,status,raw,err=fetch(url); e["listing_fetch"]={"ok":ok,"status":status,"error":err}
        if ok and raw:
            e["address_hints"]=list(dict.fromkeys(e["address_hints"]+addresses(raw)))[:5]
            e["seller_name"]=seller_name(raw)
            if private_candidate: e["public_contacts"]+=contacts(raw,url,"LISTING_ORIGINALE","HIGH")

    queries=[f'"{title[:110]}" "{comune}"' if title else f'"{comune}" vendita immobile']
    if e["address_hints"]: queries.append(f'"{e["address_hints"][0]}" "{comune}" (vendita OR appartamento OR casa OR villa)')
    seen=set()
    for q in queries[:2]:
        e["queries"].append(q); results,_error=web_search(q,8)
        for r in results:
            u=norm(r["url"])
            if u==url or u in seen: continue
            seen.add(u); ms=match_score(x,r,e["address_hints"])
            if ms<35: continue
            m={"url":u,"title":r.get("title",""),"snippet":r.get("snippet",""),"host":host(u),"match_score":ms,"contact_scan":"NOT_SCANNED"}
            if private_candidate and portal(u) and ms>=55:
                ok2,st2,raw2,er2=fetch(u); m["contact_scan"]="OK" if ok2 else f"HTTP_{st2 or 0}"
                if ok2 and raw2:
                    conf="HIGH" if ms>=75 else "MEDIUM"; e["public_contacts"]+=contacts(raw2,u,"CROSS_MATCH_IMMOBILE",conf)
                    if not e["seller_name"]: e["seller_name"]=seller_name(raw2)
    ...
    e["contact_ready"]=bool(ready); e["contact_ready_count"]=len(ready); e["cross_match_count"]=len(e["cross_matches"])
    x["enrichment"]=e
    for c in ready:
        out_rows.append({"ITEM_ID":item_id,"COMUNE":comune,"TITOLO":title,"TIPO":c.get("type",""),"VALORE":c.get("value",""),"CONFIDENZA":c.get("confidence",""),"FONTE_TIPO":c.get("source_type",""),"FONTE_URL":c.get("source_url",""),"CROSS_MATCH":e["cross_match_count"],"SELLER_NAME":e["seller_name"],"CHECKED_AT":e["checked_at"]})
    print(f"[{pos}/{len(candidates)}] {comune} | privato={private_candidate} | cross={e['cross_match_count']} | contatti={len(ready)}")
    time.sleep(0.25)

state["items"]=items; state["enrichment_updated_at"]=now(); STATE.write_text(json.dumps(state,ensure_ascii=False,indent=2),encoding="utf-8")
...
if QUEUE.exists():
    with QUEUE.open(encoding="utf-8-sig",newline="") as f:
        rows=list(csv.DictReader(f)); fields=list(rows[0].keys()) if rows else ["PRIORITA","SCORE","COMUNE","FONTE","TITOLO","PREZZO","PREZZO_PRECEDENTE","RIBASSI","INDIZIO_INSERZIONISTA","STATO","PRIMA_RILEVAZIONE","ULTIMO_CONTROLLO","MOTIVI","URL"]
    extras=["CONTATTO_PRONTO","CONTATTI_PUBBLICI","FONTE_CONTATTO","CROSS_MATCH","NOME_INSERZIONISTA","RICERCA_PUBBLICA"]
    fields += [k for k in extras if k not in fields]; by_url={norm(x.get("url","")):x for x in items.values() if x.get("url")}
    for r in rows:
        item=by_url.get(norm(r.get("URL","")),{}); e=item.get("enrichment") or {}; ready=[c for c in (e.get("public_contacts") or []) if c.get("confidence") in {"HIGH","MEDIUM"}]
        r["CONTATTO_PRONTO"]="SI" if ready else "NO"; r["CONTATTI_PUBBLICI"]=" | ".join(f"{c.get('type')}:{c.get('value')}" for c in ready[:4]); r["FONTE_CONTATTO"]=" | ".join(dict.fromkeys(c.get("source_url","") for c in ready if c.get("source_url"))); r["CROSS_MATCH"]=str(e.get("cross_match_count") or 0); r["NOME_INSERZIONISTA"]=e.get("seller_name") or ""; r["RICERCA_PUBBLICA"]="ESEGUITA" if e else "NON_ESEGUITA"
    with QUEUE.open("w",encoding="utf-8-sig",newline="") as f:
        w=csv.DictWriter(f,fieldnames=fields); w.writeheader(); w.writerows(rows)
print(f"Arricchimento v2 completato: {len(candidates)} annunci, {len(out_rows)} contatti operativi.")
