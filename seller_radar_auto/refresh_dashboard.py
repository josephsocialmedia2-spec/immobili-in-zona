...
# Giro, territorio, qualità e stato possono classificare le righe ma non aggiungerle o eliminarle dal MASTER.
...
for row in master_sorted:
    ...
    source_action = (
        f"<a class='source-link' href='{source_url}' target='_blank' rel='noopener'>APRI FONTE</a>"
    ...
    delete_action = (
        f"<button class='delete-contact' type='button' data-delete-id='{esc(master_id)}'>ELIMINA CONTATTO</button>"
    )
    rows.append(
        ...
        f"<td><div class='actions'>{source_action}{delete_action}</div></td></tr>"
    )
...
<div class='legend'><b>Ordine della lista:</b> Paese/Comune → Stato Giro → Score. Il MASTER resta preservato. Se un contatto non ti interessa, usa <b>ELIMINA CONTATTO</b>: scompare dalla tua vista ma può essere ripristinato.</div>
<div class='filters'>{''.join(filter_buttons)}</div>
<div class='visible-count'><span>VISIBILI: <span id='visibleCount'>{seller_master}</span> / {seller_master}</span><span>ELIMINATI DALLA VISTA: <span id='dismissedCount'>0</span></span><button id='restoreDismissed' class='restore-dismissed' type='button'>RIPRISTINA ELIMINATI</button></div>
...
<script>
const STORAGE_KEY='f1_seller_radar_dismissed_v1';
...
const restoreDismissed=document.getElementById('restoreDismissed');
let activeFilter='ALL';
let dismissed=new Set();
try{{
  dismissed=new Set((JSON.parse(localStorage.getItem(STORAGE_KEY)||'[]')||[]).map(String));
}}catch(e){{
  dismissed=new Set();
}}
...
function updateDismissedCount(){{
  dismissedCount.textContent=dismissed.size;
}}
function applyFilter(value){{
  activeFilter=value;
  let shown=0;
  sellerRows.forEach(row=>{{
    const masterId=String(row.dataset.masterId||'');
    const hiddenByUser=dismissed.has(masterId);
    const typeMatch=value==='ALL' || (value==='ASTE' ? row.dataset.asta==='SI' : row.dataset.tipologia===value);
    const match=!hiddenByUser && typeMatch;
...
document.querySelectorAll('.delete-contact').forEach(button=>{{
  button.addEventListener('click',()=>{{
    const masterId=String(button.dataset.deleteId||'');
    if(!masterId) return;
    if(!confirm('Eliminare questo contatto dalla tua vista Seller Radar?')) return;
    dismissed.add(masterId);
    persistDismissed();
...
restoreDismissed.addEventListener('click',()=>{{
  ...
  if(!confirm('Ripristinare tutti i contatti eliminati dalla vista?')) return;
  dismissed.clear();
  persistDismissed();
  updateDismissedCount();
  applyFilter(activeFilter);
...
if "APRI IN F1 INDIRIZZO REMOTO" in rendered:
    raise SystemExit("FAIL DASHBOARD: il collegamento F1 Indirizzo Remoto non deve più essere mostrato")
if "ELIMINA CONTATTO" not in rendered or "RIPRISTINA ELIMINATI" not in rendered:
    raise SystemExit("FAIL DASHBOARD: controlli elimina/ripristina mancanti")
...
print("PASS DASHBOARD MASTER 660 | 660 righe preservate; F1 Indirizzo Remoto rimosso; elimina contatto locale attivo")